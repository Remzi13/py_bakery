...
def seed_full_data():
    ...
    try:
        ...
        expenses = [
            ("Monthly Rent", 50000, exp_cat_rent, 4, False),
            ("Electricity", 7000, exp_cat_rent, 4, False),
            ("Ingredients Restock", 0, exp_cat_raw, 1, True),
            ("Baker Salary", 45000, exp_cat_salary, 4, False),
        ]
        
        for name, price, cat, un,  is_stock in expenses:
            if not db.query(ExpenseType).filter_by(name=name).first():
                db.add(ExpenseType(name=name, default_price=price, category_id=cat.id, unit_id=un, stock=is_stock))
        # --- 5. SUPPLIERS ---
